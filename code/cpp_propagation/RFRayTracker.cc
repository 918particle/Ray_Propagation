...
void RFRayTracker::StoreNewReflection(std::pair<float,float> angles,std::vector<float> positions,float r)
{
...
void RFRayTracker::StoreFinalData(float angle,std::vector<float> pos)
{
	_finalAngle = angle;
	_finalDepth = pos.at(2);
	_finalRange = pos.at(0);
}
void RFRayTracker::ReadoutTracker(std::string title)
{
	...
	for(int i=0;i<_numberOfReflections;++i)
	{
		...
		out<<" "<<_depthsOfReflections[i].at(0)<<" "<<_depthsOfReflections[i].at(2)<<" "<<_reflectionCoefficients[i]<<std::endl;
	}
	//Print final state
