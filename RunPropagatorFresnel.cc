...
int main(int argc, char *argv[])
{
	...
	for(float angle=angle_i;angle<=angle_f;angle+=delta_angle)
	{
		...
		r_s->InitializeReceiver(receiver_y,propagator_z,angle,timeStep);
		r_s->SetPolarization(pr);
		r_p->InitializeReceiver(receiver_y,propagator_z,angle,timeStep);
		...
		for(int i=0;i<nrays;++i)
		{
			...
			ss1<<angle;
			ss2<<i;
			m_s->ReadoutPath("data/sp_propagation_path_"+ss1.str()+"_"+ss2.str()+".dat");
			delete m_s;
